...
// Hash Monitor - A tool to track changes in a codebase by generating and comparing file hashes.
class HashMonitor {
  ...
  // ✅ New method: Initialize tracking directories
  initializeTracking() {
  ...
  // ✅ New methods: Lock and unlock the tracking directory
  lock() {
    try {
  ...
  // ✅ New method: Unlock the tracking directory
  unlock() {
    try {
  ...
  // ✅ New method: Check if a file should be ignored
  ...
  // ✅ New method: Generate a SHA-256 hash for a file
  ...
  // ✅ New method: Get all files in the project directory recursively
  ...
  // ✅ New method: Generate hashes for all files in the project
  ...
  // ✅ New method: Generate a master hash from all file hashes
  ...
  // ✅ New method: Load and save baseline hashes
  loadBaselineHashes() {
    try {
  ...
  // ✅ New method: Save master hash to file
  ...
  // ✅ New method: Save changed files to a timestamped JSON file
  ...
  // ✅ New method: Detect changes between current and baseline hashes
  ...
  // ✅ New method: Initialize the scan and establish baseline
  initializeScan() {
  ...
  // ✅ New method: Scan for changes since the last baseline
  scanForChanges() {
  ...
  // ✅ New method: Display changes in a user-friendly format
  ...
  // ✅ New method: Get the current status of the project
  getStatus() {
    const baselineHashes = this.loadBaselineHashes();
...
// Main function to handle command line arguments and execute appropriate methods
function main() {
